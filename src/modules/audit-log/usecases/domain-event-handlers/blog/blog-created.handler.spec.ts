...
import { BlogCreatedHandler } from './blog-created.handler';
...
describe('BlogCreatedHandler', () => {
  let target: BlogCreatedHandler;
  ...
  beforeEach(async () => {
    ...
    target = module.get<BlogCreatedHandler>(BlogCreatedHandler);
  });

  it('should call CreateAuditLogUseCase with correct parameters', async () => {
    // Arrange
    const event: DomainEventDto<BlogCreatedPayload> = {
      id: 'evt-123',
      code: 'ORG01001',
    ...
    // Assert
    expect(mockCreateAuditLogUseCase.execute).toHaveBeenCalledWith(event, 'Blog', 'CREATE');
