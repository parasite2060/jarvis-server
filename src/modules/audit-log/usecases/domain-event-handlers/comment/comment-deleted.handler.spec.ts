...
describe('CommentDeletedHandler', () => {
  let target: CommentDeletedHandler;
  ...
  beforeEach(async () => {
    ...
    target = module.get<CommentDeletedHandler>(CommentDeletedHandler);
  });

  it('should call CreateAuditLogUseCase with correct parameters', async () => {
    // Arrange
    const event: DomainEventDto<CommentDeletedPayload> = {
      id: 'evt-123',
      code: 'ORG02002',
    ...
    // Assert
    expect(mockCreateAuditLogUseCase.execute).toHaveBeenCalledWith(event, 'Comment', 'DELETE');
