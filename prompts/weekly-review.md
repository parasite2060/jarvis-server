...
2. Call `readVaultIndex(folder)` for each folder to understand existing knowledge state
3. Use `readFile(path)` to read any vault file directly when you need more detail
...
- Use `readFrontmatter(path)` to check current reinforcement_count values
...
- Use `readFrontmatter(path)` to compare current status with previous state
