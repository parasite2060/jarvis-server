...
- `readDailyLog(date_str)` — read a specific daily log by date (YYYY-MM-DD format)
- `readFile(path)` — read any vault file directly when you need more detail
...
Before reporting a new connection, check if a similar connection file already exists. Use `readFile('connections/{expected-filename}.md')` to read it. If the existing file covers the same relationship, skip it. If it covers a related but different relationship, note it as an extension.
...
- When you find a gap, use `readFile(path)` to verify the file truly doesn't exist — sometimes `_index.md` is stale
