...
- `summary` must be under 100 characters, one line, no markdown syntax. Describe what the file *is*, not what's inside it. Every `VaultFileEntry.summary` you produce is written into the file's frontmatter under `summary:` AND used as the `_index.md` entry text.
...
When checking if a file should be promoted, pruned, or superseded, call `readFrontmatter(path)` first to get the current status and reinforcement count. Only call `readFile(path)` when you need the full content body. This saves tokens on the most frequent operation during consolidation.
